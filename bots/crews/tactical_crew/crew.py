...
def build_tactical_crew(tools=None) -> Crew:
    ...
    plan_tactical_query_task = Task(
        description=(
            ...
            "Tool contract you are planning for (`get_tactical_order_bundle`):\n"
            "- Input `power_name`: the power to evaluate.\n"
            "- Input `annotations`: list of annotation names to include in scoring metadata.\n"
            "- Input `include_non_moves`: whether non-move legal orders are included as candidates.\n"
            "- Input `beam_width`: integer search width for deterministic bundle selection.\n\n"
            "Valid `annotations` values and meaning:\n"
            "- `supply_center_delta_if_success`: supply-center gain/loss signal for successful move destinations.\n"
            "- `adjacent_foreign_militaries_to_destination`: count of nearby foreign units around destination.\n"
            "- `immediate_contestation_count`: count of opponents with immediate access to destination.\n"
            "- `own_support_potential`: nearby friendly units that can support the action.\n"
            "- `enemy_support_potential`: nearby enemy units that can support opposition.\n"
            "- `destination_degree`: local connectivity (number of adjacent territories).\n"
            "- `holdability_next_turn`: estimated ability to keep the destination next turn.\n"
            "- `is_reversible`: whether the action can be easily undone next turn.\n"
            "- `centerity_gain`: improvement in board centrality/position.\n"
            "- `value_score`: deterministic tactical reward score.\n"
            "- `risk_score`: deterministic tactical risk score.\n"
            "- `net_score`: reward-risk combined score.\n\n"
