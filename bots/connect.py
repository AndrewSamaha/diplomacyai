...
async def list_games(hostname='localhost', port=8432):
    """ Play as the specified power """
    connection = await connect(hostname, port)
    channel = await connection.authenticate(USERNAME, PASSWORD)

    game_list = enumerate(await channel.list_games())
...
async def play(game_id, power_name, hostname='localhost', port=8432):
    ...
    game_list = await channel.list_games()
    print(game_list)
    return
...
if __name__ == '__main__':
    ...
    if not GAME_ID:
        print(f"No GAME_ID set, fetching game list")
        asyncio.run(list_games())
        exit(1)

    exit(0)
    asyncio.run(launch(game_id=str(GAME_ID)))
